...
pre_list = file_name_path(pre_path)


for subsetindex in range(len(pre_list)):
    brats_subset_path = pre_path + "/" + str(pre_list[subsetindex]) + "/"
    # 获取每个病例的四个模态及Mask的路径
    flair_image = brats_subset_path + str(pre_list[subsetindex]) + flair_name
    t1_image = brats_subset_path + str(pre_list[subsetindex]) + t1_name
    t1ce_image = brats_subset_path + str(pre_list[subsetindex]) + t1ce_name
    t2_image = brats_subset_path + str(pre_list[subsetindex]) + t2_name
    mask_image = brats_subset_path + str(pre_list[subsetindex]) + mask_name
    # 获取每个病例的四个模态及Mask数据
    ...
    print(str(pre_list[subsetindex]))
    # 切片处理,并去掉没有病灶的切片
    for n_slice in range(flair_crop.shape[0]):
        if np.max(mask_crop[n_slice, :, :]) != 0:
            ...
            imagepath = outputImg_path + "\\" + str(pre_list[subsetindex]) + "_" + str(n_slice) + ".npy"
            maskpath = outputMask_path + "\\" + str(pre_list[subsetindex]) + "_" + str(n_slice) + ".npy"
